...
class Player_model extends MY_Model {
	...
	public function get_player($playerid, $esportid)
	{
		...
		$sql = "SELECT p.*
				...
				LIMIT 1";
		$result = $this->db1->query($sql);
		$player_result = $result->row_array();
		...
		//Check to see if play is in a team
		$sql = "SELECT pt.teamid, t.team_name
				FROM player_teams pt, teams t
				WHERE pt.playerid = '$playerid'
					AND pt.teamid = t.teamid";
		$result = $this->db1->query($sql);
		$player_teams = $result->result_array();
		...
		if(!empty($player_teams))
		{
			...
			{
				array_push($player['teams'], $team['teamid']);
